import React from "react";

const NodeJsDevelopment = () => {
  return (
    <>
      <div className="wrapper-content">
        <div className="fw-page-builder-content">
          {/* title start */}
          <div className="section section-2">
            <div className="hero">
              <div className="container hero-section">
                <div className="left-block">
                  <h1
                    style={{
                      fontSize: "36px",
                      fontWeight: "bold",
                      color: "rgb(0, 117, 255)",
                      textAlign: "center",
                      textTransform: "uppercase",
                    }}
                  >
                    <span style={{ color: "#fff" }}>
                      Paradigm Shifting Technology:
                    </span>{" "}
                    <br />
                    Node JS
                  </h1>
                </div>
                <div className="right-block">
                  <img src="../assets/images/services/service.png" alt="" />
                </div>
              </div>
            </div>
          </div>
          {/* title end */}

          {/* node js start */}
          <div
            className="section section-2 nodejs "
            id="intro"
            style={{ backgroundColor: "#f6f6f6" }}
          >
            <div className="align-left">
              <div className="row">
                <div className="col-xs-12 col-sm-4 equal-cols no-padding">
                  <img
                    alt="//steadyrabbit.in/wp-content/uploads/2021/11/Product-Engineering-Services.png"
                    data-src="//steadyrabbit.in/wp-content/uploads/2021/11/Product-Engineering-Services.png"
                    className="lazyload"
                    src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                  />
                  <noscript>
                    <img
                      src="../wp-content/uploads/2021/11/Product-Engineering-Services.png"
                      alt="//steadyrabbit.in/wp-content/uploads/2021/11/Product-Engineering-Services.png"
                    />
                  </noscript>
                </div>
                <div className="col-xs-12 col-sm-4 equal-cols no-padding">
                  <div
                    className="info-box-item"
                    style={{ backgroundColor: "#f6f6f6" }}
                  >
                    <div className="info-box-item__inner">
                      <div className="info-box__description">
                        <h1
                          className="info-box__title"
                          style={{
                            color: "#0075FF",
                            textTransform: "uppercase",
                            fontWeight: "bold",
                          }}
                        >
                          Node JS Development
                        </h1>
                        <p>
                          The non-blocking nature of Node.js functions enables
                          concurrent and simultaneous command execution.
                          Applications that demand many concurrent connections
                          and are highly scalable should use it. A server-side
                          runtime environment that is open-source and
                          cross-platform is called Node.js.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-xs-12 col-sm-4 equal-cols no-padding">
                  <div
                    className="info-box-item"
                    style={{ backgroundColor: "#f6f6f6" }}
                  >
                    <div className="info-box-item__inner">
                      <div className="info-box-2-subitem">
                        <div className="info-box-2-subitem__description">
                          <p>
                            <i
                              className="fa fa-check"
                              style={{ color: "#0a0039", marginLeft: "-28px" }}
                              aria-hidden="true"
                            ></i>
                             
                          </p>
                          <p style={{ marginLeft: "0px", marginTop: "-24px" }}>
                            Ideal for developing real-time web applications
                          </p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <div className="info-box-2-subitem__description">
                          <p>
                            <i
                              className="fa fa-check"
                              style={{ color: "#0a0039", marginLeft: "-28px" }}
                              aria-hidden="true"
                            ></i>
                             
                          </p>
                          <p style={{ marginLeft: "0px", marginTop: "-24px" }}>
                            Support for high-load web applications that works
                            effectively
                          </p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <div className="info-box-2-subitem__description">
                          <p>
                            <i
                              className="fa fa-check"
                              style={{ color: "#0a0039", marginLeft: "-28px" }}
                              aria-hidden="true"
                            ></i>
                             
                          </p>
                          <p style={{ marginLeft: "0px", marginTop: "-24px" }}>
                            An event-driven, non-blocking I/O model
                          </p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <div className="info-box-2-subitem__description">
                          <p>
                            <i
                              className="fa fa-check"
                              style={{ color: "#0a0039", marginLeft: "-28px" }}
                              aria-hidden="true"
                            ></i>
                             
                          </p>
                          <p style={{ marginLeft: "0px", marginTop: "-24px" }}>
                            Elegant and straightforward opportunity for code
                            sharing, updating, and reuse
                          </p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <div className="info-box-2-subitem__description">
                          <p>
                            <i
                              className="fa fa-check"
                              style={{ color: "#0a0039", marginLeft: "-28px" }}
                              aria-hidden="true"
                            ></i>
                             
                          </p>
                          <p style={{ marginLeft: "0px", marginTop: "-24px" }}>
                            Construction of a network application that is quick
                            and flexible
                          </p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          {/* node js end */}

          {/* services offer start */}
          <section className="section section-white section-2" id="service111">
            <div className="container align-left">
              <div className="row">
                <div className="col-xs-12 col-sm-12">
                  <h3 className="section__title">Our Service Offerings</h3>
                </div>
              </div>
            </div>

            <div className="container align-left">
              <div className="row equal-cols-container">
                <div
                  className=" col-xs-12 col-sm-4 col-md-4 col-lg-4 equal-cols"
                  // style={{ height: "221px" }}
                >
                  <div
                    className="features-item"
                    style={{ textAlign: "center" }}
                  >
                    <img
                      alt="Product Engineering"
                      data-src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61485.png"
                      className="features-item__icon  ls-is-cached lazyloaded"
                      src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61485.png"
                    />
                    <noscript>
                      <img
                        src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61485.png"
                        className="features-item__icon "
                        alt="Product Engineering"
                      />
                    </noscript>

                    <div className="features-item__text">
                      <h5>
                        <a
                          href="/product-engineering-services/"
                          target="_blank"
                          rel="noopener"
                          style={{ color: "#0a0039 " }}
                        >
                          Custom Node JS Development
                        </a>
                      </h5>
                      <p style={{ marginBottom: "5%", textAlign: "justify" }}>
                        We offer Node.js programmers who are skilled in
                        developing websites and mobile apps. We'll run it for
                        you, whether it's a Node.js module or an end-to-end
                        solution.
                      </p>
                    </div>
                  </div>
                </div>
                <div className=" col-xs-12 col-sm-4 col-md-4 col-lg-4 equal-cols">
                  <div
                    className="features-item"
                    style={{ textAlign: "center" }}
                  >
                    <img
                      alt="Mobility"
                      data-src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61486.png"
                      className="features-item__icon  lazyloaded"
                      src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61486.png"
                    />
                    <noscript>
                      <img
                        src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61486.png"
                        className="features-item__icon "
                        alt="Mobility"
                      />
                    </noscript>

                    <div className="features-item__text">
                      <h5>
                        <a
                          href="/custom-mobile-application-development/"
                          target="_blank"
                          rel="noopener"
                          style={{ color: "#0a0039 " }}
                        >
                          Node JS Mobile Development
                        </a>
                      </h5>
                      <p style={{ marginBottom: "5%", textAlign: "justify" }}>
                        OneAD, our previous mobile gaming experience, scaled to
                        30 million users. You may be confident that we will
                        provide you with high-quality solutions for your mobile
                        needs.
                      </p>
                    </div>
                  </div>
                </div>
                <div className=" col-xs-12 col-sm-4 col-md-4 col-lg-4 equal-cols">
                  <div
                    className="features-item"
                    style={{ textAlign: "center" }}
                  >
                    <img
                      alt="Analytics"
                      data-src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61487.png"
                      className="features-item__icon  lazyloaded"
                      src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61487.png"
                    />
                    <noscript>
                      <img
                        src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61487.png"
                        className="features-item__icon "
                        alt="Analytics"
                      />
                    </noscript>

                    <div className="features-item__text">
                      <h5>
                        <a
                          href="/business-analytics-services/"
                          target="_blank"
                          rel="noopener"
                          style={{ color: "#0a0039 " }}
                        >
                          Migrate to Node JS
                        </a>
                      </h5>
                      <p style={{ marginBottom: "5%", textAlign: "justify" }}>
                        Our Node.js programmers assist you in redesigning any
                        and all solutions. Regardless of the old technology, our
                        Node.js developers are trained to migrate the legacy
                        solution.
                      </p>
                    </div>
                  </div>
                </div>
                <div className=" col-xs-12 col-sm-4 col-md-4 col-lg-4 equal-cols">
                  <div
                    className="features-item"
                    style={{ textAlign: "center" }}
                  >
                    <img
                      alt="Cloud And Devops"
                      data-src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61488.png"
                      className="features-item__icon  lazyloaded"
                      src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61488.png"
                    />
                    <noscript>
                      <img
                        src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61488.png"
                        className="features-item__icon "
                        alt="Cloud And Devops"
                      />
                    </noscript>

                    <div className="features-item__text">
                      <h5>
                        <a
                          href="/cloud-management-and-devops-services/"
                          target="_blank"
                          rel="noopener"
                          style={{ color: "#0a0039 " }}
                        >
                          Node JS Support & Maintenance
                        </a>
                      </h5>
                      <p style={{ marginBottom: "5%", textAlign: "justify" }}>
                        We never want your systems or solutions to fail. The
                        Node.js support team works to give you fast, frequent
                        updates on how your system is performing.
                      </p>
                    </div>
                  </div>
                </div>
                <div className=" col-xs-12 col-sm-4 col-md-4 col-lg-4 equal-cols">
                  <div
                    className="features-item"
                    style={{ textAlign: "center" }}
                  >
                    <img
                      alt="Re-Engineering"
                      data-src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61489.png"
                      className="features-item__icon  lazyloaded"
                      src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61489.png"
                    />
                    <noscript>
                      <img
                        src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61489.png"
                        className="features-item__icon "
                        alt="Re-Engineering"
                      />
                    </noscript>

                    <div className="features-item__text">
                      <h5>
                        <a
                          href="/migration-upgrade-and-product-re-engineering/"
                          target="_blank"
                          rel="noopener"
                          style={{ color: "#0a0039 " }}
                        >
                          Node JS Web Development
                        </a>
                      </h5>
                      <p style={{ marginBottom: "5%", textAlign: "justify" }}>
                        Web development and Node.js application integration are
                        significantly simplified by our Node.js outsourcing
                        team. The development and deployment will be phased out
                        by Node.js engineers.
                      </p>
                    </div>
                  </div>
                </div>
                <div className=" col-xs-12 col-sm-4 col-md-4 col-lg-4 equal-cols">
                  <div
                    className="features-item"
                    style={{ textAlign: "center" }}
                  >
                    <img
                      alt="Testing"
                      data-src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61490.png"
                      className="features-item__icon  lazyloaded"
                      src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61490.png"
                    />
                    <noscript>
                      <img
                        src="https://steadyrabbit.in/sr/wp-content/uploads/2021/01/Group-61490.png"
                        className="features-item__icon "
                        alt="Testing"
                      />
                    </noscript>

                    <div className="features-item__text">
                      <h5>
                        <a
                          href="/mobile-web-application-testing-services/"
                          target="_blank"
                          rel="noopener"
                          style={{ color: "#0a0039 " }}
                        >
                          Upgrade Node JS Version
                        </a>
                      </h5>
                      <p style={{ marginBottom: "5%", textAlign: "justify" }}>
                        Every project requires improvement. Your project will be
                        updated to the most recent Node.js version by our
                        Node.js developers to keep it on the leading edge.
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </section>
          {/* services offer end */}

          {/* tech expert start */}
          <div
            className="section hidden-xs section-2 pt-0 pb-0"
            id="Technology"
            style={{ backgroundColor: "#f6f6f6" }}
          >
            <div className="align-left">
              <div className="row">
                <div className="col-xs-12 col-sm-4 equal-cols no-padding">
                  <div
                    className="info-box-item"
                    style={{ backgroundColor: "#f6f6f6" }}
                  >
                    <div className="info-box-item__inner">
                      <h5
                        className="info-box__title"
                        style={{
                          color: "#0075FF",
                          textTransform: "uppercase",
                        }}
                      >
                        Web Development Technical Expertise
                      </h5>
                    </div>
                  </div>
                </div>
                <div className="col-xs-12 col-sm-4 equal-cols no-padding">
                  <div
                    className="info-box-item"
                    style={{ backgroundColor: "#f6f6f6" }}
                  >
                    <div className="info-box-item__inner">
                      <div className="info-box-2-subitem">
                        <img
                          alt=""
                          data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Frontend-UX.png"
                          className="info-box-2-subitem__ico lazyload"
                          src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                        />
                        <noscript>
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Frontend-UX.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              src="../wp-content/uploads/2021/09/Frontend-UX.png"
                              className="info-box-2-subitem__ico"
                              alt=""
                            />
                          </noscript>
                        </noscript>

                        <span
                          className="info-box-2-subitem__title"
                          style={{ color: "#0a0039" }}
                        >
                          Frontend &amp; UX
                        </span>
                        <div className="info-box-2-subitem__description">
                          <p>
                            React.js, AngularJS, VueJS, React Native, Flutter,
                            Android, iOS
                          </p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <img
                          alt=""
                          data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Backend.png"
                          className="info-box-2-subitem__ico lazyload"
                          src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                        />
                        <noscript>
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Backend.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              src="../wp-content/uploads/2021/09/Backend.png"
                              className="info-box-2-subitem__ico"
                              alt=""
                            />
                          </noscript>
                        </noscript>

                        <span
                          className="info-box-2-subitem__title"
                          style={{ color: "#0a0039" }}
                        >
                          Backend
                        </span>
                        <div className="info-box-2-subitem__description">
                          <p>Java, Python, Node.Js, PHP</p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <img
                          alt=""
                          data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Database.png"
                          className="info-box-2-subitem__ico lazyload"
                          src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                        />
                        <noscript>
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Database.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              src="../wp-content/uploads/2021/09/Database.png"
                              className="info-box-2-subitem__ico"
                              alt=""
                            />
                          </noscript>
                        </noscript>
                        <span
                          className="info-box-2-subitem__title"
                          style={{ color: "#0a0039" }}
                        >
                          Database
                        </span>
                        <div className="info-box-2-subitem__description">
                          <p>MongoDB, SQL, Redis</p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <img
                          alt=""
                          data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Analytics.png"
                          className="info-box-2-subitem__ico lazyload"
                          src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                        />
                        <noscript>
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Analytics.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              src="../wp-content/uploads/2021/09/Analytics.png"
                              className="info-box-2-subitem__ico"
                              alt=""
                            />
                          </noscript>
                        </noscript>
                        <span
                          className="info-box-2-subitem__title"
                          style={{ color: "#0a0039" }}
                        >
                          Analytics
                        </span>
                        <div className="info-box-2-subitem__description">
                          <p>Panda, R, Hadoop, Hive</p>
                        </div>
                      </div>
                      <div className="info-box-2-subitem">
                        <img
                          alt=""
                          data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Cloud-and-DevOps.png"
                          className="info-box-2-subitem__ico lazyload"
                          src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                        />
                        <noscript>
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Cloud-and-DevOps.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              src="../wp-content/uploads/2021/09/Cloud-and-DevOps.png"
                              className="info-box-2-subitem__ico"
                              alt=""
                            />
                          </noscript>
                        </noscript>
                        <span
                          className="info-box-2-subitem__title"
                          style={{ color: "#0a0039" }}
                        >
                          Cloud And Devops
                        </span>
                        <div className="info-box-2-subitem__description">
                          <p>AWS, Jenkins, Docker, Kubernet</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-xs-12 col-sm-4 equal-cols no-padding">
                  <img
                    alt="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                    data-src="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                    className="lazyload"
                    src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                  />
                  <noscript>
                    <img
                      alt="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                      data-src="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                      className="lazyload"
                      src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                    />
                    <noscript>
                      <img
                        src="../wp-content/uploads/2021/09/Technical-Expertise-1.png"
                        alt="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                      />
                    </noscript>
                  </noscript>
                </div>
              </div>
            </div>
          </div>
          <div
            className="section hidden-sm hidden-md hidden-lg section-2 pt-0 pb-0"
            id="Technology-Mob"
            style={{ backgroundColor: "#f6f6f6" }}
          >
            <div className="container-middle">
              <div className="container align-left">
                <div className="row">
                  <div className="col-xs-12 col-sm-4 no-padding">
                    <div
                      className="info-box-item"
                      style={{ backgroundColor: "#f6f6f6" }}
                    >
                      <div className="info-box-item__inner">
                        <h5
                          className="info-box__title"
                          style={{
                            color: "#0075FF",
                            textTransform: "uppercase",
                          }}
                        >
                          Web Development Technical Expertise
                        </h5>
                      </div>
                    </div>

                    <div
                      className="info-box-item"
                      style={{ backgroundColor: "#f6f6f6" }}
                    >
                      <div className="info-box-item__inner">
                        <div className="info-box-2-subitem">
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Frontend-UX.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt=""
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Frontend-UX.png"
                              className="info-box-2-subitem__ico lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                src="../wp-content/uploads/2021/09/Frontend-UX.png"
                                className="info-box-2-subitem__ico"
                                alt=""
                              />
                            </noscript>
                          </noscript>

                          <span
                            className="info-box-2-subitem__title"
                            style={{ color: "#0a0039" }}
                          >
                            Frontend &amp; UX
                          </span>
                          <div className="info-box-2-subitem__description">
                            <p>
                              React.js , AngularJS , VueJS, React Native ,
                              Flutter, Android , iOS
                            </p>
                          </div>
                        </div>
                        <div className="info-box-2-subitem">
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Backend.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt=""
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Backend.png"
                              className="info-box-2-subitem__ico lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                src="../wp-content/uploads/2021/09/Backend.png"
                                className="info-box-2-subitem__ico"
                                alt=""
                              />
                            </noscript>
                          </noscript>

                          <span
                            className="info-box-2-subitem__title"
                            style={{ color: "#0a0039" }}
                          >
                            Backend
                          </span>
                          <div className="info-box-2-subitem__description">
                            <p>Java, Python, Node.Js, PHP</p>
                          </div>
                        </div>
                        <div className="info-box-2-subitem">
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Database.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt=""
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Database.png"
                              className="info-box-2-subitem__ico lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                src="../wp-content/uploads/2021/09/Database.png"
                                className="info-box-2-subitem__ico"
                                alt=""
                              />
                            </noscript>
                          </noscript>

                          <span
                            className="info-box-2-subitem__title"
                            style={{ color: "#0a0039" }}
                          >
                            Database
                          </span>
                          <div className="info-box-2-subitem__description">
                            <p>MongoDB, SQL, Redis</p>
                          </div>
                        </div>
                        <div className="info-box-2-subitem">
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Analytics.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt=""
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Analytics.png"
                              className="info-box-2-subitem__ico lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                src="../wp-content/uploads/2021/09/Analytics.png"
                                className="info-box-2-subitem__ico"
                                alt=""
                              />
                            </noscript>
                          </noscript>
                          <span
                            className="info-box-2-subitem__title"
                            style={{ color: "#0a0039" }}
                          >
                            Analytics
                          </span>
                          <div className="info-box-2-subitem__description">
                            <p>Panda, R, Hadoop, Hive</p>
                          </div>
                        </div>
                        <div className="info-box-2-subitem">
                          <img
                            alt=""
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Cloud-and-DevOps.png"
                            className="info-box-2-subitem__ico lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt=""
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/09/Cloud-and-DevOps.png"
                              className="info-box-2-subitem__ico lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                src="../wp-content/uploads/2021/09/Cloud-and-DevOps.png"
                                className="info-box-2-subitem__ico"
                                alt=""
                              />
                            </noscript>
                          </noscript>
                          <span
                            className="info-box-2-subitem__title"
                            style={{ color: "#0a0039" }}
                          >
                            Cloud And Devops
                          </span>
                          <div className="info-box-2-subitem__description">
                            <p>AWS, Jenkins, Docker, Kubernet</p>
                          </div>
                        </div>
                      </div>
                    </div>

                    <img
                      alt="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                      data-src="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                      className="lazyload"
                      src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                    />
                    <noscript>
                      <img
                        alt="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                        data-src="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                        className="lazyload"
                        src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                      />
                      <noscript>
                        <img
                          src="../wp-content/uploads/2021/09/Technical-Expertise-1.png"
                          alt="//steadyrabbit.in/wp-content/uploads/2021/09/Technical-Expertise-1.png"
                        />
                      </noscript>
                    </noscript>
                  </div>
                </div>
              </div>
            </div>
          </div>
          {/* tech expert end */}

          {/* success stories start */}
          <section className="section section-white section-2 pb-0">
            <div className="container align-left">
              <div className="row">
                <div className="col-xs-12 col-sm-12">
                  <h3
                    className="section__title"
                    style={{
                      color: "#0075ff",
                      textTransform: "uppercase",
                    }}
                  >
                    Success Stories
                  </h3>
                </div>
              </div>
            </div>

            <div className="container align-left">
              <div className="row">
                <div className="col-xs-12 col-sm-3 equal-cols">
                  <div className="team-item">
                    <div className="team-item__wrap">
                      <img
                        alt="OneAD"
                        data-src="//steadyrabbit.in/wp-content/uploads/2021/01/Group-63109.jpg"
                        className="team-item__img lazyload"
                        src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                      />
                      <noscript>
                        <img
                          src="wp-content/uploads/2021/01/Group-63109.jpg"
                          alt="OneAD"
                          className="team-item__img"
                        />
                      </noscript>
                      <div className="team-item-social">
                        <a
                          className="team-item-social__link"
                          target="_blank"
                          href="onead/index.htm"
                        >
                          <img
                            alt="readmore"
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                            className="team-item-social__icon lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt="readmore"
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                              className="team-item-social__icon lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                alt="readmore"
                                data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                className="team-item-social__icon lazyload"
                                src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                              />
                              <noscript>
                                <img
                                  alt="readmore"
                                  data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                  className="team-item-social__icon lazyload"
                                  src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                                />
                                <noscript>
                                  <img
                                    src="wp-content/uploads/2021/01/readmore.png"
                                    className="team-item-social__icon"
                                    alt="readmore"
                                  />
                                </noscript>
                              </noscript>
                            </noscript>
                          </noscript>
                        </a>
                      </div>
                    </div>
                    <div className="team-item-details">
                      <div
                        className="team-item-details__name"
                        style={{ color: "#0a0039" }}
                      >
                        Mobile Gaming Platform
                      </div>
                      <div className="team-item-details__position">
                        App loved by 30 Mn users
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-xs-12 col-sm-3 equal-cols">
                  <div className="team-item">
                    <div className="team-item__wrap">
                      <img
                        alt="Event Creative Platform"
                        data-src="//steadyrabbit.in/wp-content/uploads/2022/01/Yogems.png"
                        className="team-item__img lazyload"
                        src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                      />
                      <noscript>
                        <img
                          src="wp-content/uploads/2022/01/Yogems.png"
                          alt="Event Creative Platform"
                          className="team-item__img"
                        />
                      </noscript>
                      <div className="team-item-social">
                        <a
                          className="team-item-social__link"
                          target="_blank"
                          href="yogems/index.htm"
                        >
                          <img
                            alt="readmore"
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                            className="team-item-social__icon lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt="readmore"
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                              className="team-item-social__icon lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                alt="readmore"
                                data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                className="team-item-social__icon lazyload"
                                src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                              />
                              <noscript>
                                <img
                                  alt="readmore"
                                  data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                  className="team-item-social__icon lazyload"
                                  src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                                />
                                <noscript>
                                  <img
                                    src="wp-content/uploads/2021/01/readmore.png"
                                    className="team-item-social__icon"
                                    alt="readmore"
                                  />
                                </noscript>
                              </noscript>
                            </noscript>
                          </noscript>
                        </a>
                      </div>
                    </div>
                    <div className="team-item-details">
                      <div
                        className="team-item-details__name"
                        style={{ color: "#0a0039" }}
                      >
                        Event Creative Platform
                      </div>
                      <div className="team-item-details__position">
                        Arts, Craft, Photography &amp; Music
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-xs-12 col-sm-3 equal-cols">
                  <div className="team-item">
                    <div className="team-item__wrap">
                      <img
                        alt="Group Shopping App"
                        data-src="//steadyrabbit.in/wp-content/uploads/2022/01/MALL91.png"
                        className="team-item__img lazyload"
                        src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                      />
                      <noscript>
                        <img
                          src="wp-content/uploads/2022/01/MALL91.png"
                          alt="Group Shopping App"
                          className="team-item__img"
                        />
                      </noscript>
                      <div className="team-item-social">
                        <a
                          className="team-item-social__link"
                          target="_blank"
                          href="mall91/index.htm"
                        >
                          <img
                            alt="readmore"
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                            className="team-item-social__icon lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt="readmore"
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                              className="team-item-social__icon lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                alt="readmore"
                                data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                className="team-item-social__icon lazyload"
                                src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                              />
                              <noscript>
                                <img
                                  alt="readmore"
                                  data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                  className="team-item-social__icon lazyload"
                                  src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                                />
                                <noscript>
                                  <img
                                    src="wp-content/uploads/2021/01/readmore.png"
                                    className="team-item-social__icon"
                                    alt="readmore"
                                  />
                                </noscript>
                              </noscript>
                            </noscript>
                          </noscript>
                        </a>
                      </div>
                    </div>
                    <div className="team-item-details">
                      <div
                        className="team-item-details__name"
                        style={{ color: "#0a0039" }}
                      >
                        Group Shopping App
                      </div>
                      <div className="team-item-details__position">
                        Built for the next 100 Cr. Indian users
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-xs-12 col-sm-3 equal-cols">
                  <div className="team-item">
                    <div className="team-item__wrap">
                      <img
                        alt="Real Estate App"
                        data-src="//steadyrabbit.in/wp-content/uploads/2022/01/UAV-Real-Estate.png"
                        className="team-item__img lazyload"
                        src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                      />
                      <noscript>
                        <img
                          src="wp-content/uploads/2022/01/UAV-Real-Estate.png"
                          alt="Real Estate App"
                          className="team-item__img"
                        />
                      </noscript>
                      <div className="team-item-social">
                        <a
                          className="team-item-social__link"
                          target="_blank"
                          href="uav-real-estate/index.htm"
                        >
                          <img
                            alt="readmore"
                            data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                            className="team-item-social__icon lazyload"
                            src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                          />
                          <noscript>
                            <img
                              alt="readmore"
                              data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                              className="team-item-social__icon lazyload"
                              src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                            />
                            <noscript>
                              <img
                                alt="readmore"
                                data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                className="team-item-social__icon lazyload"
                                src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                              />
                              <noscript>
                                <img
                                  alt="readmore"
                                  data-src="https://steadyrabbit.in/wp-content/uploads/2021/01/readmore.png"
                                  className="team-item-social__icon lazyload"
                                  src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                                />
                                <noscript>
                                  <img
                                    src="wp-content/uploads/2021/01/readmore.png"
                                    className="team-item-social__icon"
                                    alt="readmore"
                                  />
                                </noscript>
                              </noscript>
                            </noscript>
                          </noscript>
                        </a>
                      </div>
                    </div>
                    <div className="team-item-details">
                      <div
                        className="team-item-details__name"
                        style={{ color: "#0a0039" }}
                      >
                        Real Estate App
                      </div>
                      <div className="team-item-details__position">
                        Search homes remotely
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </section>
          {/* success stories end */}

          {/* development process start */}
          <section
            className="section hidden-xs section-lightgray section-2 pb-0"
            id="process"
            style={{
              background: "rgb(246, 246, 246)",
            }}
          >
            <div className="container align-left">
              <div className="row">
                <div className="col-xs-12 col-sm-12">
                  <h3 className="section__title" style={{ color: "#0075FF" }}>
                    Development Process
                  </h3>
                </div>
              </div>
            </div>

            <div className="container align-left">
              <div className="row">
                <div className=" col-xs-12 col-sm-12">
                  <img
                    alt="//steadyrabbit.in/wp-content/uploads/2021/09/development-process.jpg"
                    data-src="//steadyrabbit.in/wp-content/uploads/2021/09/development-process.jpg"
                    className=" ls-is-cached lazyloaded"
                    src="//steadyrabbit.in/wp-content/uploads/2021/09/development-process.jpg"
                  />
                  <noscript>
                    <img
                      src="//steadyrabbit.in/wp-content/uploads/2021/09/development-process.jpg"
                      alt="//steadyrabbit.in/wp-content/uploads/2021/09/development-process.jpg"
                    />
                  </noscript>
                </div>
              </div>
            </div>
          </section>
          {/* development process end */}

          {/* get in touch start */}
          <div className="section pt-0 pb-0" id="cta">
            <div className="align-left">
              <div className="s-get-it-2">
                <div className="container">
                  <div className="row">
                    <div className="col-xs-12 col-sm-12 col-md-8 align-middle">
                      <h5 className="s-get-it-2__title">
                        Get In Touch <br /> Be a part of the solution!
                      </h5>
                    </div>
                    <div className="col-xs-12 col-sm-12 col-md-4 align-middle">
                      <a
                        href="contact-us/index.htm"
                        target="_blank"
                        className="btn-a btn-a_fill_theme"
                        style={{
                          borderRadius: "5px",
                          borderColor: "#0075ff",
                        }}
                      >
                        Get quote
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          {/* get in touch end */}
        </div>
      </div>
    </>
  );
};

export default NodeJsDevelopment;
