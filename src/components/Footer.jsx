import React from "react";

const Footer = () => {
  return (
    <>
      <footer className="footer" id="footer">
        <div className="footer-top">
          <div className="container">
            <div className="row">
              <div className="col-xs-12 col-sm-6 col-md-3 pb-50">
                <div
                  id="custom_html-2"
                  className="widget_text widget widget-footer widget_custom_html"
                >
                  <div className="textwidget custom-html-widget">
                    <p>&nbsp;</p>
                    <a href="index.htm">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="110.698"
                        height="78.447"
                        viewBox="0 0 110.698 78.447"
                        style={{ paddingTop: "7px", paddingBottom: "7px" }}
                      >
                        <g id="Logo" transform="translate(-140.69 -24.469)">
                          <g
                            id="Group_62872"
                            data-name="Group 62872"
                            transform="translate(1.052 0.339)"
                          >
                            <path
                              id="Path_95636"
                              data-name="Path 95636"
                              d="M3.027-4.471V-.679H.765V-10.812H4.939q2.007,0,2.007,1.753v2.836A1.535,1.535,0,0,1,5.672-4.535L7.457-.679H5.019L3.426-4.471Zm0-4.668V-6.08H4.254a.467.467,0,0,0,.319-.088.385.385,0,0,0,.1-.295V-8.741q0-.4-.414-.4ZM21.78-.679l-.351-1.832h-2.31L18.769-.679H16.522l2.294-10.133h2.9L24.074-.679ZM19.438-4.168h1.657l-.828-4.286ZM37.76-6.956v-1.9q0-.4-.43-.4H36.024v2.693H37.33Q37.76-6.558,37.76-6.956Zm0,4.318V-4.614a.385.385,0,0,0-.1-.295A.5.5,0,0,0,37.33-5H36.024V-2.24H37.33Q37.76-2.24,37.76-2.639Zm-3.983-8.173h4.206q2.023,0,2.023,1.753v1.784q0,1.179-.717,1.482.717.271.717,1.37v1.976q0,1.769-2.023,1.769H33.777ZM54.266-6.956v-1.9q0-.4-.43-.4H52.53v2.693h1.306Q54.266-6.558,54.266-6.956Zm0,4.318V-4.614a.385.385,0,0,0-.1-.295A.5.5,0,0,0,53.836-5H52.53V-2.24h1.306Q54.266-2.24,54.266-2.639Zm-3.983-8.173h4.206q2.023,0,2.023,1.753v1.784q0,1.179-.717,1.482.717.271.717,1.37v1.976q0,1.769-2.023,1.769H50.283Zm16.554,0H69.1V-.679H66.837Zm18.02,0v1.673H82.993v8.46H80.714v-8.46H78.866v-1.673Z"
                              transform="translate(152 103.256)"
                              fill="#1b1e22"
                            ></path>
                          </g>
                          <path
                            id="Path_95637"
                            data-name="Path 95637"
                            d="M13.731-4.807a4.579,4.579,0,0,1-.724,2.53A4.946,4.946,0,0,1,10.9-.495,7.6,7.6,0,0,1,7.613.161,8.032,8.032,0,0,1,2.795-1.208,5.041,5.041,0,0,1,.736-5.221H6.164q.115,1.4,1.242,1.4a.938.938,0,0,0,.633-.207.744.744,0,0,0,.241-.6,1.013,1.013,0,0,0-.587-.909A11.536,11.536,0,0,0,5.8-6.3,17.683,17.683,0,0,1,3.232-7.337,5.133,5.133,0,0,1,1.449-8.9,4.44,4.44,0,0,1,.69-11.592a4.45,4.45,0,0,1,.817-2.7,5.042,5.042,0,0,1,2.231-1.691,8.393,8.393,0,0,1,3.186-.575A7.314,7.314,0,0,1,11.569-15.2a5.118,5.118,0,0,1,1.886,3.91h-5.5Q7.843-12.581,6.9-12.581a.739.739,0,0,0-.529.2.77.77,0,0,0-.207.587.984.984,0,0,0,.587.862,12.063,12.063,0,0,0,1.829.771,19.624,19.624,0,0,1,2.588,1.092,5.311,5.311,0,0,1,1.794,1.587A4.4,4.4,0,0,1,13.731-4.807ZM31.993-16.284v4.048H27.669V0H22.563V-12.236H18.285v-4.048Zm10.166,4.048v2.024h5.06v3.841h-5.06v2.3h5.75V0H37.053V-16.284H47.909v4.048Zm21.8,9.775H58.535L57.73,0H52.371l5.957-16.284h5.888L70.15,0H64.768ZM62.721-6.3l-1.472-4.531L59.777-6.3ZM90.229-8.188A8.472,8.472,0,0,1,89.194-4a7.45,7.45,0,0,1-2.955,2.933A9.044,9.044,0,0,1,81.765,0H74.98V-16.284h6.785a9.288,9.288,0,0,1,4.474,1.035,7.265,7.265,0,0,1,2.955,2.875A8.39,8.39,0,0,1,90.229-8.188ZM81.305-4.554A3.855,3.855,0,0,0,84.042-5.5a3.5,3.5,0,0,0,1.012-2.691,3.5,3.5,0,0,0-1.012-2.691,3.855,3.855,0,0,0-2.737-.943H80.086v7.268Zm30.084-11.73L105.547-4.945V0h-5.106V-4.945L94.6-16.284h5.842l2.6,5.773,2.6-5.773Z"
                            transform="translate(140 84.784)"
                            fill="#f11b27"
                          ></path>
                          <g
                            id="Group_61392"
                            data-name="Group 61392"
                            transform="translate(156 24.88)"
                          >
                            <g
                              id="Group_61391"
                              data-name="Group 61391"
                              transform="translate(0 0)"
                            >
                              <g
                                id="Group_61280"
                                data-name="Group 61280"
                                transform="translate(0.73 0) rotate(1)"
                              >
                                <g
                                  id="Group_61261"
                                  data-name="Group 61261"
                                  transform="translate(0 0)"
                                >
                                  <path
                                    id="Path_12"
                                    data-name="Path 12"
                                    d="M73.431,41.829c-.1-.039-.23-.088-.4-.149A39.531,39.531,0,0,1,54.464,27.171h0a.422.422,0,0,1,.5-.637h0c3.417,1.38,9.389,3.379,11,1.261.977-1.286.02-3.915-1.377-5.735-1.767-2.18-5.6-4.088-9.858-6.053a.426.426,0,0,0-.51.124c-1.944,2.407-4.821,3.49-8.765,3.3-3.674-.172-7.666-1.38-11.527-2.549-3.736-1.131-7.264-2.2-9.675-1.989-5.1.443-6.09,4.388-7.133,8.565a21.685,21.685,0,0,1-2.075,5.885C10.443,37.015.428,37.661,0,37.685c-.231.012,9.949-1.492,14.318-8.772a21.1,21.1,0,0,0,1.98-5.655c1.069-4.28,2.174-8.706,7.878-9.2,2.572-.223,6.176.867,9.992,2.022,7.1,2.149,15.1,4.572,19.271-.334.217-.278.17-.243-.085-.368-4.272-1.956-8.719-3.973-11.772-6.4C38.549,6.564,37.03,4.406,37.5,2.111A2.53,2.53,0,0,1,39.214.186c2.143-.656,4.791.526,7.923,1.9C52,4.227,55.24,6.515,56.228,9.082a5.9,5.9,0,0,1-.992,5.686.422.422,0,0,0,.168.624c4.219,1.956,8,3.875,9.845,6.146a10.4,10.4,0,0,1,1.726,3.355,3.81,3.81,0,0,1-.34,3.413c-1.249,1.644-4.08,1.558-7.84.47a.422.422,0,0,0-.451.661A43.891,43.891,0,0,0,73.431,41.829ZM41.206,1.1c-1.75-.109-2.767,1.022-2.319,2.516,1.281,4.272,7.821,7.791,14.592,10.891l.365.167a.422.422,0,0,0,.519-.138c1.368-1.9,2.039-4.59,1.372-6.187C53.826,3.773,43.123,1.181,41.206,1.1Z"
                                    transform="translate(2.321 0)"
                                    stroke="#1b1e22"
                                    strokeMiterlimit="10"
                                    strokeWidth="1"
                                  ></path>
                                  <path
                                    id="Path_13"
                                    data-name="Path 13"
                                    d="M63.165,7.642a18.043,18.043,0,0,1-4.05.4c-9.462-.1-18.265-2.077-26.778-3.986C25.8,2.6,19.634,1.213,13.849.873a36.059,36.059,0,0,0-13.84,1.9C-.3,2.863,7.395-.351,13.9.032c5.854.344,12.056,1.734,18.622,3.207,8.471,1.9,17.23,3.863,26.6,3.966a27.136,27.136,0,0,0,3.92-.4c4.312-.623,9.68-1.4,14.421,1.857C77.656,8.8,73,5.436,63.165,7.642Z"
                                    transform="translate(0 26.983)"
                                    stroke="#1b1e22"
                                    strokeWidth="1"
                                  ></path>
                                  <ellipse
                                    id="Ellipse_1"
                                    data-name="Ellipse 1"
                                    cx="0.857"
                                    cy="0.857"
                                    rx="0.857"
                                    ry="0.857"
                                    transform="translate(61.497 21.197)"
                                  ></ellipse>
                                </g>
                              </g>
                            </g>
                          </g>
                        </g>
                      </svg>
                    </a>
                  </div>
                </div>
                <div
                  id="footer-contacts-10"
                  className="widget widget-footer widget_footer-contacts"
                >
                  <ul className="footer-contacts"></ul>
                  <div className="footer-social">
                    <a href="https://fb.me/Steadyrabbit" target="_blank">
                      <i className="ico-7 footer-social__item"></i>
                    </a>
                    <a href="https://twitter.com/SteadyRabbits" target="_blank">
                      <i className="ico-6 footer-social__item"></i>
                    </a>
                    <a
                      href="https://www.linkedin.com/company/steady-rabbit-technology"
                      target="_blank"
                    >
                      <i className="ico-2 footer-social__item"></i>
                    </a>
                  </div>
                </div>
                <div
                  id="custom_html-3"
                  className="widget_text widget widget-footer widget_custom_html"
                >
                  <div className="textwidget custom-html-widget">
                    <img
                      data-src="/wp-content/uploads/2021/09/email.png"
                      className="lazyload"
                      src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                    />
                    <noscript>
                      <img src="wp-content/uploads/2021/09/email.png" />
                    </noscript>
                    <p
                      style={{
                        marginTop: "-37px",
                        marginLeft: "36px",
                        fontSize: "14px",
                        color: "#000000",
                        fontWeight: "500",
                      }}
                    >
                      <a
                        href="mailto:contact@steadyrabbit.in"
                        style={{
                          fontSize: "14px",
                          color: "#000000",
                          fontWeight: "500",
                        }}
                      >
                        contact@SteadyRabbit.in
                      </a>
                    </p>
                    <img
                      data-src="/wp-content/uploads/2021/01/IND-Flag.png"
                      className="lazyload"
                      src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                    />
                    <noscript>
                      <img src="wp-content/uploads/2021/01/IND-Flag.png" />
                    </noscript>
                    <p
                      style={{
                        marginTop: "-37px",
                        marginLeft: "36px",
                        fontSize: "14px",
                        color: "#ffffff",
                        fontWeight: "500",
                      }}
                    >
                      <a href="tel:+91 931-007-0057">+91 931-007-0057</a>
                    </p>
                    <img
                      data-src="/wp-content/uploads/2021/01/USA-Flag.png"
                      className="lazyload"
                      src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                    />
                    <noscript>
                      <img src="wp-content/uploads/2021/01/USA-Flag.png" />
                    </noscript>
                    <p
                      style={{
                        marginTop: "-37px",
                        marginLeft: "36px",
                        fontSize: "14px",
                        color: "#ffffff",
                        fontWeight: "500",
                      }}
                    >
                      <a href="tel:+1 609-454-2480">+1 609-454-2480</a>
                    </p>
                    <img
                      width="25px"
                      height="18px"
                      data-src="/wp-content/uploads/2021/02/UK-Flag.png"
                      className="lazyload"
                      src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw=="
                    />
                    <noscript>
                      <img
                        src="wp-content/uploads/2021/02/UK-Flag.png"
                        width="25px"
                        height="18px"
                      />
                    </noscript>
                    <p
                      style={{
                        marginTop: "-37px",
                        marginLeft: "36px",
                        fontSize: "14px",
                        color: "#ffffff",
                        fontWeight: "500",
                      }}
                    >
                      <a href="tel:+44 203-287-8632">+44 203-287-8632</a>
                    </p>
                  </div>
                </div>
              </div>

              <div className="col-xs-12 col-sm-6 col-md-3 pb-50">
                <div id="text-5" className="widget widget-footer widget_text">
                  <h4 className="widget-title footer__title">Company</h4>
                  <div className="textwidget">
                    <p>
                      <a href="/services">Services</a>
                    </p>
                    <p>
                      <a href="/success-stories">Success Stories</a>
                    </p>
                    <p>
                      <a href="/about-us">About Us</a>
                    </p>
                    <p>
                      <a href="/contact-us">Contact us</a>
                    </p>
                    <p>
                      <a href="/blog">Blog</a>
                    </p>
                  </div>
                </div>
              </div>

              <div className="col-xs-12 col-sm-6 col-md-3 pb-50">
                <div id="text-6" className="widget widget-footer widget_text">
                  <div className="textwidget">
                    <h4 className="widget-title footer__title">
                      <a href="/services">SERVICES</a>
                    </h4>
                    <p>
                      <a href="/services/mobile-application-development">
                        Mobile Application Development
                      </a>
                    </p>
                    <p>
                      <a href="/services/devops-solution-and-services">
                        DevOps Solutions & Services
                      </a>
                    </p>
                    <p>
                      <a href="/services/product-re-engineering">
                        Product Re-Engineering
                      </a>
                    </p>
                  </div>
                </div>
              </div>

              <div className="col-xs-12 col-sm-6 col-md-3 pb-50">
                <div id="text-7" className="widget widget-footer widget_text">
                  <div className="textwidget">
                    <h4 className="widget-title footer__title">
                      <a href="application-development-technology-offerings/index.htm">
                        TECHNOLOGY
                      </a>
                    </h4>
                    <p>
                      <a href="custom-nodejs-application-development/index.htm">
                        Nodejs
                      </a>
                    </p>
                    <p>
                      <a href="custom-python-application-development/index.htm">
                        Python
                      </a>
                    </p>
                    <p>
                      <a href="custom-java-application-development/index.htm">
                        Java
                      </a>
                    </p>
                    <p>
                      <a href="android-custom-application-development/index.htm">
                        Android
                      </a>
                    </p>
                    <p>
                      <a href="custom-ios-application-development/index.htm">
                        iOS
                      </a>
                    </p>
                    <p>
                      <a href="custom-react-application-development/index.htm">
                        Reactjs
                      </a>
                    </p>
                    <p>
                      <a href="react-native/index.htm">React Native</a>
                    </p>
                    <p>
                      <a href="blockchain-development-services/index.htm">
                        Blockchain
                      </a>
                    </p>
                    <p>
                      <a href="angular/index.htm">Angular</a>
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="footer-bottom">
          <div className="container">
            <div className="row">
              <ul className="footer-copy col-xs-12 col-sm-7 col-md-8">
                <li>
                  © Steady Rabbit All Right Reserved.&nbsp;
                  <a
                    className="footer-menu__link"
                    href="privacy-policy/index.htm"
                  >
                    Privacy Policy
                  </a>
                </li>
                {/* <li><a className="likeapro" href="//themeforest.net/item/nowadays-multipurpose-onemultipage-wordpress-theme/18399207" target="_blank" title="Nowadays - Multipurpose WordPress Theme">Nowadays</a> - WordPress Theme. Developed &amp; Designed by <a href="https://likeaprothemes.com" target="_blank" title="Premium WordPress themes" className="likeapro">like@prothemes</a></li> */}
              </ul>
              <div></div>
            </div>
          </div>
        </div>
      </footer>
    </>
  );
};

export default Footer;
